...
#    for alpha in 2; do
#      for scenario in INC_TASK INC_DOMAIN INC_CLASS; do
#        python train.py cfg=default_cifar100 cfg.seed=${seed} cfg.lwf_alpha=${alpha} \
#          cfg.lwf_temperature=${temperature} cfg.scenario=${scenario} tags=["${scenario}"] >/dev/null 2>&1 &
#        echo "Running LWF on ${scenario} with seed = ${seed}, alpha = ${alpha}, temp. = ${temperature}"
#      done
#      wait
#    done
#  done
