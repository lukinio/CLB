...
import models
from interval.hyperparam_scheduler import StepScheduler
...
class IntervalNet(nn.Module):
    def __init__(self, agent_config):
        ...
        self.current_total_eps = self.config['init_eps']
    ...
    def batch_wc_acc(self, outputs, target, _tasks):
        correct, total = 0, 0
        # should we recompute these outputs with eval?
        # orig_mode = self.training
        # self.eval()
        ...
        if self.multihead:
            ...
            for t, t_logits in logits.items():
                ...
                if len(inds) > 0:
                    ...
                    if self.kappa_scheduler.current < 1.0:
    ...
    def update_model(self, inputs, targets, tasks):
        self.model.importances_to_eps(self.current_total_eps, mode=self.eps_mode)
        out = self.forward(inputs)
        loss, robust_err, robust_loss, ce_loss = self.criterion(out, targets, tasks)
        wc_acc_estimate = self.batch_wc_acc(out, targets, tasks)
        if self.kappa_scheduler.current < 1.0:
            wc_threshold = self.config['wc_threshold'][self.current_task - 1]
            if wc_acc_estimate > wc_threshold:
                # TODO parameterize if necessary
                self.current_total_eps *= 1.2
            # elif wc_acc_estimate < 1 - wc_threshold:
            #     self.current_total_eps /= 1.2
        self.zero_grad()
        loss.backward()
        ...
        for t in out.keys():
            out[t], _, _ = split_activation(out[t])
            out[t] = out[t].detach()
        return loss.item(), robust_err, robust_loss.item(), ce_loss.item(), out, wc_acc_estimate
    ...
    def learn_batch(self, train_loader, val_loader=None):
        ...
        for epoch in range(schedule):
            ...
            for i, (inputs, target, task) in enumerate(train_loader):
                ...
                loss, robust_err, robust_loss, ce_loss, output, wc_acc_estimate = self.update_model(inputs, target, task)
            ...
            if is_wandb_on:
                wandb.log({
                    ...
                    'total_epsilon': self.current_total_eps,
                    'robust_loss': robust_loss,
                    'robust_error': robust_err,
                    'wc_acc_estimate': wc_acc_estimate,
                },
